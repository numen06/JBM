                ...
                post: function postLink(scope) {
                    ...
                    var renderBreadcrumb = function() {
                        ...
                        angular.forEach(scope.steps, function (step) {
                            if(step.ncyBreadcrumb.isUrl)
                            {
                                step.ncyBreadcrumb.label=$rootScope.attrName;
                            }
                            if (step.ncyBreadcrumb && step.ncyBreadcrumb.label) {

                                if(step.ncyBreadcrumb.labels&&step.ncyBreadcrumb.labels.length>0)
                                {
                                    angular.forEach(step.ncyBreadcrumb.labels, function (la) {
//                                var parseLabel = $interpolate(la.label);
//                                step.ncyBreadcrumbLabel = parseLabel(viewScope);
                                        scope.arr.push({ncyBreadcrumbLabel:la.label,ncyBreadcrumbLink:la.url});
                                        // Watcher for further viewScope updates
//                                registerWatchers(labelWatchers, la.label, viewScope, step);
                                    });
                                }
                                else
                                {
                                    scope.arr.push({ncyBreadcrumbLabel:step.ncyBreadcrumb.label,ncyBreadcrumbLink:""});
                                }
