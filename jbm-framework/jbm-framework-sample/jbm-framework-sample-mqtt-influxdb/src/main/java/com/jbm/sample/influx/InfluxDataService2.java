...
@Service
public class InfluxDataService2 {
	private final static Logger logger = LoggerFactory.getLogger(InfluxDataService2.class);
	...
	@PostConstruct
	public void init() {
		SimulationDataService.dbTempEventBus.register(this);
	}

	@Subscribe
	public void revData(JSONObject data) {
		try {
			BatchPoints batchPoints = BatchPoints.database(dbName).retentionPolicy("one_day").build();
			Builder b = Point.measurement("datatemp").time(TimeUtil.softParseDate(data.getString("time"), new Date()).getTime(), TimeUnit.MILLISECONDS)
