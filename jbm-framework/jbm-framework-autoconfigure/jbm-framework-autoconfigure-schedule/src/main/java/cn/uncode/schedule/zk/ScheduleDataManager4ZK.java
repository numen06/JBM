...
public class ScheduleDataManager4ZK implements IScheduleDataManager {
	...
	public ScheduleDataManager4ZK(ZKManager aZkManager) throws Exception {
		...
		// gson = new GsonBuilder().registerTypeAdapter(Timestamp.class, new
	...
	public boolean refreshScheduleServer(ScheduleServer server) throws Exception {
		...
		if (this.getZooKeeper().exists(zkPath, false) == null) {
			...
			byte[] jsonBytes = JSON.toJSONBytes(server, SerializerFeature.DisableCircularReferenceDetect);
			try {
				this.getZooKeeper().setData(zkPath, jsonBytes, -1);
			} catch (Exception e) {
				// 恢复上次的心跳时间
	...
	public void registerScheduleServer(ScheduleServer server) throws Exception {
		...
		// SerializerFeature.DisableCircularReferenceDetect);
		byte[] jsonBytes = JSON.toJSONBytes(server, SerializerFeature.DisableCircularReferenceDetect);
		this.getZooKeeper().setData(realPath, jsonBytes, -1);
	...
	public void addTask(TaskDefine taskDefine) throws Exception {
		...
		if (null == data || data.length == 0) {
			...
			byte[] jsonBytes = JSON.toJSONBytes(taskDefine, SerializerFeature.DisableCircularReferenceDetect);
			this.getZooKeeper().setData(zkPath, jsonBytes, -1);
		}
	}
	...
	public void updateTask(TaskDefine taskDefine) throws Exception {
		...
		if (this.getZooKeeper().exists(zkPath, false) == null) {
			...
			if (null != data) {
				String json = new String(data);
				LOG.debug(json);
				tmpTaskDefine = JSON.parseObject(json, TaskDefine.class);
				tmpTaskDefine.valueOf(tmpTaskDefine);
			} else {
				tmpTaskDefine = new TaskDefine();
			}
			tmpTaskDefine.valueOf(taskDefine);
			// String json = JSON.toJSONString(tmpTaskDefine,
			// SerializerFeature.DisableCircularReferenceDetect);
			byte[] jsonBytes = JSON.toJSONBytes(taskDefine, SerializerFeature.DisableCircularReferenceDetect);
			this.getZooKeeper().setData(zkPath, jsonBytes, -1);
		}
				...
				for (String taskName : children) {
					if (isOwner(taskName, currentUuid)) {
						...
						if (null != data) {
							String json = new String(data);
							LOG.debug(json);
							TaskDefine td = JSON.parseObject(json, TaskDefine.class);
