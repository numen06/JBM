...
public class MqttMessage {
    ...
    public String getPayloadStr() {
        return StrUtil.str(payload, CharsetUtil.UTF_8);
    }
    ...
    public static MqttMessage of(String topic) {
        if (StrUtil.isBlank(topic)) {
            throw new IllegalArgumentException("payload is blank");
        }
        return new MqttMessage(topic, null);
    }

    public static MqttMessage ofJson(String topic, Object payload) {
        if (StrUtil.isBlank(topic)) {
            throw new IllegalArgumentException("payload is blank");
        }
        return new MqttMessage(topic, JSON.toJSONString(payload).getBytes());
    }
