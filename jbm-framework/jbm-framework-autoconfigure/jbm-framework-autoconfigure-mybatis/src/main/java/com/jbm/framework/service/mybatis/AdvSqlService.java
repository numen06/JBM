...
public class AdvSqlService<Entity extends AdvEntity> extends BaseSqlService<Entity>
	...
	public Map<String, Entity> selectEntityMapByCodes(Collection<String> codes) throws DataServiceException {
		try {
			...
			return MapUtils.fromList(list, IAdvSqlService.CODE_COLUMN, String.class);
		...
		try (SqlSession batchSqlSession = sqlSessionBatch()) {
			for (Entity anEntityList : entityList) {
				...
				if (null != tableInfo && StringUtils.isNotEmpty(tableInfo.getKeyProperty())) {
					Object codeVal = ReflectionKit.getMethodValue(cls, anEntityList, IAdvSqlService.CODE_COLUMN);
	...
	private QueryWrapper<Entity> buildWrapperByCode(String code) {
		...
		queryWrapper.eq(IAdvSqlService.CODE_COLUMN, code);
		return queryWrapper;
	}

	private QueryWrapper<Entity> buildWrapperByCodes(Collection<String> codes) {
		QueryWrapper<Entity> queryWrapper = new QueryWrapper<>();
		queryWrapper.in(IAdvSqlService.CODE_COLUMN, codes);
		return queryWrapper;
	}
}
