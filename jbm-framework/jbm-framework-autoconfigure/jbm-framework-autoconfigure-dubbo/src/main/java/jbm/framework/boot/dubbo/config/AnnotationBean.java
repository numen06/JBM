package jbm.framework.boot.dubbo.config;
...
import jbm.framework.boot.dubbo.annotation.Inject;
	...
	public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
		...
		if (service != null) {
			if (void.class.equals(service.interfaceClass()) && "".equals(service.interfaceName())) {
				...
				Assert.notEmpty(interfaces, "Failed to export remote service class " + clazs.getName()
	...
	private void buildField(Object bean) {
		ReflectionUtils.doWithFields(this.getOriginalClass(bean), field -> {
			try {
				...
				logger.error("Failed to init remote service reference at filed {} in class {}, cause: {}", field.getName(), this.getOriginalClass(bean).getName(), e.getMessage(),
					e);
		...
		ReflectionUtils.doWithMethods(this.getOriginalClass(bean), method -> {
			if (method.getParameterTypes().length == 1 && Modifier.isPublic(method.getModifiers()) && !Modifier.isStatic(method.getModifiers())) {
				try {
					...
					logger.error("Failed to init remote service reference at method {} in class {}, cause: {}", method.getName(), this.getOriginalClass(bean).getName(),
						e.getMessage(), e);
