...
| `recUserId = 0`（线上超级管理员） | `jbm-user-0` |
| 未指定收件人 | 按标准发送入口发送给当前登录用户 |

重复收件人去重；真实用户 0 不会回退为当前用户或被解释成广播。队列消息携带多人列表时也按人拆分。
...
手机可以通过 WSS 订阅自己的用户主题。群发使用标准 `recUserIds` 收件人列表，不保留特殊广播用户 ID。
...
线上验证标准发送入口 → RabbitMQ → Push 消费者 → 独立用户 topic 的 WSS 接收和消息记录。
后续核对真实账号发现超级管理员 ID 为 0，现已修正为 `jbm-user-0`，不能将其当作广播。
