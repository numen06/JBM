...
import com.td.framework.service.FileSupportService;
import com.td.util.Base64Utils;
import com.td.util.CollectionUtils;
import com.td.util.PathUtils;
...
public class MongoFileSupportService implements FileSupportService {
	...
	public MongoFileSupportService(GridFsTemplate gridFsTemplate) {
		super();
	...
	private FileInfoBean buildFileInfoBean(GridFSFile file) {
		FileInfoBean fileBean = new FileInfoBean(file.getId().toString(), file.getFilename(), file.getContentType(), file.getLength(), file.getChunkSize(), file.getUploadDate(),
			file.getMD5(), null);
		if (file.getMetaData() != null) {
			fileBean.setMetaData(file.getMetaData().toMap());
	...
	public FileInfoBean saveFile(FileInfoBean fileInfoBean) throws IOException {
		...
		return buildFileInfoBean(file);
	}

	public FileInfoBean saveFile(boolean randomName, InputStream inputStream, String originalName, String contentType) throws IOException {
		...
		GridFSFile file = gridFsTemplate.store(inputStream, saveName, contentType, metadata);
		return buildFileInfoBean(file);
	}

	@Override
	public Boolean exits(FileInfoBean fileInfoBean) {
		GridFSDBFile file = this.getFile(fileInfoBean);
		return (file != null);
	}
	...
	public InputStream getInputStream(String fileHandler) {
		return getFile(fileHandler).getInputStream();
	}

	public Long writeTo(String fileHandler, File file) throws IOException {
		return getFile(fileHandler).writeTo(file);
	}

	public Long writeTo(String fileHandler, String file) throws IOException {
		return getFile(fileHandler).writeTo(file);
	}

	@Override
	public Long writeTo(String fileHandler, OutputStream outputStream) throws IOException {
		GridFSDBFile file = getFile(fileHandler);
		if (file != null)
	...
	public Long writeTo(FileInfoBean fileInfoBean, OutputStream outputStream) throws IOException {
		GridFSDBFile file = getFile(fileInfoBean);
		if (file != null)
			return file.writeTo(outputStream);
		return null;
	}
	...
	public FileInfoBean writeToFileInfoBean(FileInfoBean fileInfoBean) throws IOException {
		...
		// 将原信息取出来放进去
		if (file.getMetaData() != null)
			db_fileInfoBean.setMetaData(file.getMetaData().toMap());
		if (file != null) {
			try {
	...
	public GridFSDBFile getFile(FileInfoBean fileInfoBean) {
		...
		return CollectionUtils.firstResult(gridFsTemplate.find(query));
	}
