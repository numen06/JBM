#!/usr/bin/env python3
"""Center 涓氬姟鍦烘櫙 REST 娴嬭瘯锛堝吀鍨嬫祦绋?+ 鏂█锛宲rofile jaja7锛夈€?""
import argparse
import json
import os
import re
...
def check_assertions(rules, ctx, jb):
    """瑙勫垯绀轰緥: eq:result.roleCode:{roleCode}; sizeGte:result:1; isFalse:success"""
    if not rules:
        return True, ""
    failures = []
    for rule in rules:
        ...
        if op in ("eq", "neq", "sizeGte", "gte"):
            ...
            if op == "eq" and not _cmp_eq(actual, expected):
                failures.append(f"{rule} 瀹為檯={actual!r}")
            elif op == "neq" and _cmp_eq(actual, expected):
                failures.append(f"{rule} 涓嶅簲绛変簬 {expected!r}")
            elif op == "sizeGte":
                n = int(expected)
                if not isinstance(actual, list) or len(actual) < n:
                    failures.append(f"{rule} 闀垮害={len(actual) if isinstance(actual, list) else 'N/A'}")
            elif op == "gte":
                try:
                    if actual is None or float(actual) < float(expected):
                        failures.append(f"{rule} 瀹為檯={actual!r}")
                except (TypeError, ValueError):
                    failures.append(f"{rule} 闈炴暟瀛?{actual!r}")
        elif op == "notNull":
            if json_path(jb, rest) is None:
                failures.append(f"{rule} 涓?null")
        elif op == "notEmpty":
            v = json_path(jb, rest)
            if v is None or v == "" or v == [] or v == {}:
                failures.append(f"{rule} 涓虹┖")
        elif op == "isList":
            v = json_path(jb, rest)
            if not isinstance(v, list):
                failures.append(f"{rule} 闈炲垪琛?)
        elif op == "isFalse":
            v = jb.get(rest) if rest == "success" and jb else json_path(jb, rest)
            if v is not False and v != 0:
                failures.append(f"{rule} 瀹為檯={v!r}")
        elif op == "isTrue":
            v = jb.get(rest) if rest == "success" and jb else json_path(jb, rest)
...
def run_step(step, cfg, ctx, scenario_id):
    ...
    if status == 0:
        ...
        if jb and jb.get("success") is True:
            ...
            msg = (jb or {}).get("message") or "鍙€夊満鏅湭婊¤冻锛堝鏃?Token/鏃犵粍缁囨暟鎹級"
    elif expect == "fail":
        if jb and jb.get("success") is False:
            ...
            msg = (jb or {}).get("message") or "鏈熸湜澶辫触浣嗚繑鍥炴垚鍔?
        if ok and asserts:
...
def write_cases(mod, path):
    lines = [
        f"# {mod['title']} - 涓氬姟鍦烘櫙鐢ㄤ緥",
        "",
        "璇存槑锛氭瘡鍦烘櫙鍚墠缃潯浠躲€佹楠や笌**涓氬姟鏂█**锛堥潪浠?HTTP 200锛夈€?,
        "",
    ]
    for sc in mod.get("scenarios") or []:
        lines.append(f"## {sc.get('id', '')} {sc.get('title', '')}")
        lines.append("")
        lines.append(f"**鍓嶇疆鏉′欢**锛歿sc.get('precondition', '-')}")
        lines.append("")
        lines.append("| 姝ラ | 鎿嶄綔 | 鏂规硶 | 璺緞 | 涓氬姟鏂█ |")
        lines.append("|------|------|------|------|----------|")
        for st in sc.get("steps") or []:
            asserts = st.get("assert") or []
            a = "; ".join(asserts) if asserts else "鎺ュ彛 success=true"
            lines.append(
...
def write_report(mod, results, path, run_time, service_ok):
    ...
    lines = [
        f"# {mod['title']} - 涓氬姟娴嬭瘯鎶ュ憡",
        "",
        f"- 鏃堕棿: {run_time}",
        f"- 鏈嶅姟鍙敤: {'鏄? if service_ok else '鍚?}",
        f"- 缁撴灉: **{'PASS' if ok else 'FAIL'}**",
        f"- 姝ラ閫氳繃: {passed}/{total}" + (f"锛堣烦杩?{skipped}锛? if skipped else ""),
        "",
        "| 鍦烘櫙 | 姝ラ | 鏂规硶 | HTTP | 缁撴灉 | 鑰楁椂(s) | 涓氬姟鏂█ | 澶囨敞 |",
        "|------|------|------|------|------|---------|----------|------|",
    ]
    for r in results:
        lines.append(
...
def main():
    ap = argparse.ArgumentParser(description="Center 涓氬姟鍦烘櫙 REST 娴嬭瘯")
    ...
    lines = [
        "# Center 涓氬姟鍦烘櫙娴嬭瘯姹囨€?(profile jaja7)",
        "",
        f"- 鏃堕棿: {run_time}",
        f"- 鍦板潃: {cfg['base_url']}",
        f"- 鏈嶅姟鍙敤: {'鏄? if service_ok else '鍚?}",
        "- 璇存槑: 鐢ㄤ緥鎸?*鍏稿瀷涓氬姟鍦烘櫙**缂栨帓锛屾楠ゅ惈瀛楁绾ф柇瑷€锛堥潪浠?success=true锛?,
        "",
        "| 妯″潡 | 姝ラ鏁?| 閫氳繃 | 缁撴灉 |",
        "|------|--------|------|------|",
    ]
    ...
    lines += [
        ...
        f"## 鎬讳綋: {overall}",
        "",
        "鎵ц: `python scripts/run_center_rest_tests.py`",
        "甯?Token: `python scripts/run_center_rest_tests.py --token <Authorization>`",
        "",
    ]
