...
public abstract class ClassUtils {
     ...
     * @return the original thread context ClassLoader, or <code>null</code> if not
     * overridden
     */
     ...
     * class loader, or the ClassLoader that loaded the ClassUtils class as
     * fallback.
     *
    ...
    /**
     * Replacement for <code>Class.forName()</code> that also returns Class
     * instances for primitives (e.g."int") and array class names (e.g. "String[]").
     ...
     * present and can be loaded. Will return <code>false</code> if either the class
    ...
    public static String getShortBeanName(Class<?> clazz) {
        return getShortNameAsProperty(clazz);
    }
     ...
     * the <code>java.lang.String</code> class.
     *
     * @param clazz the class
    ...
    /**
     * Determine whether the given class has a method with the given signature, and
     * return it if available (else throws an <code>IllegalStateException</code>).
     * <p>
     * Essentially translates <code>NoSuchMethodException</code> to
    ...
    /**
     * Determine whether the given class has a method with the given signature, and
     * return it if available (else return <code>null</code>).
     * <p>
     * Essentially translates <code>NoSuchMethodException</code> to
     * <code>null</code>.
     *
     ...
     * there is one. E.g. the method may be <code>IFoo.bar()</code> and the target
     * class may be <code>DefaultFoo</code>. In this case, the method may be
     * <code>DefaultFoo.bar()</code>. This enables attributes on that method to be
     * found.
     * <p>
     ...
     * specified class file, converting all dots ('.') to slashes ('/'), adding a
     * trailing slash if necessary, and concatenating the specified resource name to
     * this. <br/>
     ...
     * <code>ClassLoader.getResource()</code>. For it to be fed to
     * <code>Class.getResource</code> instead, a leading slash would also have to be
     ...
     * given array.
     * <p>
     * Basically like <code>AbstractCollection.toString()</code>, but stripping the
     ...
     * @return the Class array (<code>null</code> if the passed-in Collection was
     * <code>null</code>)
     */
    ...
    /**
     * Return all interfaces that the given class implements as Set, including ones
