...
@Service
public class PasswordAuthenticate implements ILoginAuthenticate {
    ...
    public ResultBody<JbmLoginUser> login(String username, String password, String loginType) {
        return ResultBody.callback(() -> {
            loginDatabaseHook.assertNotLocked(username, loginType);
            String typeParam = LoginType.PASSWORD.toString().equals(loginType) ? null : loginType.toLowerCase();
            UserAccount account = authUserBusiness.login(username, typeParam);
            if (account == null) {
                if (LoginType.MINIAPP.toString().equals(loginType)) {
                    ...
                    if (Boolean.TRUE.equals(stringRedisTemplate.hasKey(key))) {
                        ...
                        account = authUserBusiness.registerAccountByPhone(phone, username, password, loginType.toLowerCase());
                    }
                } else {
                    throw new ServiceException("没有找到此用户");
                }
            }
            if (LoginType.MINIAPP.toString().equals(loginType) || LoginType.WECHAT.toString().equals(loginType)) {
