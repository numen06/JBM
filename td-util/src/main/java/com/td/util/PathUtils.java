...
public class PathUtils extends org.apache.commons.io.FilenameUtils {
	...
	public static String randomTimeFileName(String ext) {
		...
		if (StringUtils.isEmpty(getStandardExtension(ext)))
			return TimeUtil.format(time, "yyyyMMddHHmmssSSS");
		else
			return new StringBuffer(TimeUtil.format(time, "yyyyMMddHHmmssSSS")).append(EXTENSION_SEPARATOR).append(getStandardExtension(ext)).toString();
	}
