...
public abstract class ClassUtils {
	...
	public static Class<?> getSuperClassGenricType(final Class<?> clazz, final Integer index) {
		...
		if (index >= params.length || index < 0) {
			Assert.hasLength("Index: " + index + ", Size of " + clazz.getSimpleName() + "'s Parameterized Type: " + params.length);
			return Object.class;
		}
		if (!(params[index] instanceof Class)) {
			Assert.isInstanceOf(params[index].getClass(), params[index], " not set the actual class on superclass generic parameter");
			return Object.class;
		}
