...
public class AdminClientProperties {
	...
	public void onApplicationReady(ApplicationReadyEvent event) {
		...
		managementPort = event.getApplicationContext().getEnvironment().getProperty("local.management.port", Integer.class, serverPort);
		ready = true;
	}
	...
	public String getManagementUrl() {
		...
		if ((managementPort == null || managementPort.equals(serverPort)) && getServiceUrl() != null) {
			return append(append(getServiceUrl(), server.getServletPrefix()), management.getContextPath());
		}
		...
		if (preferIp) {
			Assert.notNull(management.getAddress(), "management.address must be set when using preferIp");
			return append(append(createLocalUri(management.getAddress().getHostAddress(), managementPort), server.getContextPath()), management.getContextPath());

		}
		return append(append(createLocalUri(getHostname(), managementPort), server.getContextPath()), management.getContextPath());
	}
	...
	public String getServiceUrl() {
		...
		if (preferIp) {
			...
			return append(createLocalUri(server.getAddress().getHostAddress(), serverPort), server.getContextPath());
