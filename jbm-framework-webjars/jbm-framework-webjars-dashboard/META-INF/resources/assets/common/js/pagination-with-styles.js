...
//                $(".pagination").remove();
                ...
                var d=container.data('pagination');
            ...
            createTemplate: function(args){
                ...
                var isUseMemory=attributes.isUseMemory;
                var isPopUp=attributes.isPopUp;
                ...
                if(!self.model.totalNumber||self.model.totalNumber==0||self.model.totalNumber<self.model.pageSize)
                {
//                    $(".pagination").remove
                    return;
                }
...
//                            html += '<a data-num="'+ (currentPage - 1) +'" title="Previous page"  href="'+ pageLink +'">'+ prevText +'<\/a>';
                        }
                    }
                    // Page numbers
                    if(showPageNumbers){
...
//                                    html += '<a data-num="'+ i +'">'+ i +'<\/a>';
                                }
                                else{
                                    html += '<li class="'+ classPrefix +'-page J-paginationjs-page" data-num="'+ i +'"><a href="'+ pageLink +'">'+ i +'<\/a><\/li>';
//                                    html += '<a data-num="'+ i +'" href="'+ pageLink +'">'+ i +'<\/a>';
                                }
                            }
                        }
                        else{
...
//                                html += ' data-num="1"><a data-num="1"href="'+ pageLink +'">1<\/a><\/li>';
                            }
                        ...
                        for(i = rangeStart; i <= rangeEnd; i++){
                            if(i == currentPage){
                                html += '<li class="'+ classPrefix +'-page J-paginationjs-page '+ activeClassName +'" data-num="'+ i +'"><a class="btn-primary">'+ i +'<\/a><\/li>';
                            }
                            else{
                                html += '<li class="'+ classPrefix +'-page J-paginationjs-page" data-num="'+ i +'"><a href="'+ pageLink +'">'+ i +'<\/a><\/li>';
                            }
...
//                formatAjaxParams.data.currPage = formatAjaxParams.data.pageNumber;

                if(formatAjaxParams.data.pageNumber!=1)
                {
                    currentMemoryPage=formatAjaxParams.data.pageNumber;
                }
                else
                {
                    if(!attributes.isPopUp)
                    {
                        currentMemoryPage=1;
                    }
                }
                formatAjaxParams.data.currPage = currentMemoryPage;
                delete formatAjaxParams.data.pageNumber;

//                var map={};
//                map["pageForm"]={currPage:formatAjaxParams.data.currPage,pageNumber:formatAjaxParams.data.pageNumber,pageSize:formatAjaxParams.data.pageSize,sortRule:formatAjaxParams.data.sortRule};
//                formatAjaxParams.data.currPage=null;
//                formatAjaxParams.data.pageNumber=null;
//                formatAjaxParams.data.pageSize=null;
//                formatAjaxParams.data.sortRule=null;
//
//
//                map["data"]=formatAjaxParams.data
...
//                    render(self.filterDataByLocator(response));
                ...
                function render(data){
                    ...
                    if(attributes.isUseMemory)
                    {
                        model.pageNumber = currentMemoryPage;
                    }
                    else
                    {
                    ...
                    attributes.isPopUp=false;
                    if(!data.success)
                    {
                        return;
                    }
                    model.totalNumber = data.result.total;
                    model.totalPage = data.result.totalPage;
                    ...
                    container.data('pagination').currentPageData = data.result.list;
    ...
    function addStyle(){
        ...
        var cssText = '.paginationjs{line-height:1.6;font-family:"Marmelad","Lucida Grande","Arial","Hiragino Sans GB",Georgia,sans-serif;font-size:14px;box-sizing:initial}' +
            '.paginationjs:after{display:table;content:" ";clear:both}.paginationjs .paginationjs-pages{margin-top:-1}' +
            '.paginationjs .paginationjs-pages ul{text-align:center;margin:0;padding:0}' +
            '.paginationjs .paginationjs-pages li{border-left: 1px solid #ccc; border-top: 1px solid #ccc;border-bottom: 1px solid #ccc;;display:inline-block !important; display:-moz-grid;border-right-width:1;list-style:none}' +
