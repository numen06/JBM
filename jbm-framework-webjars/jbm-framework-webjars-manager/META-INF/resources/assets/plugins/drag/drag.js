 ...
 * @constructor
 */
function Drag(elem, event, GPU,dropList) {

    dropList=dropList;
    if (elem) {
        return new Drag.create(elem, event, GPU,dropList);
    }
...
Drag.create = function(elem, event, GPU,dropList) {

    var $elem = $(elem);
    var drag = this;
    var dragEvent = new Drag(null,null,null,dropList);
    ...
    dragEvent.onend = function(event) {
        var style = elem.style;
        var divHeight=$("#jgDiv").height();
        var divWidth=$("#jgDiv").width();

        if (drag.GPU) {
            style[transform] = '';
            style.left = ((x + startLeft)/divWidth*100).toFixed(2) + '%';
            style.top =((y + startTop)/divHeight*100).toFixed(2) + '%';
        } else {
            style.left =  (x/divWidth*100).toFixed(2) + '%';
            style.top = (y/divHeight*100).toFixed(2) + '%';
        }
        dropList[$(elem).attr("num")].left= style.left;
        dropList[$(elem).attr("num")].top= style.top;
        drag.onend(event);
    };
